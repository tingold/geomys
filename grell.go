...
//	sσ1, cσ1 = hat(sσ1, cσ1)
//	k2 := e2 * cγ0 * cγ0
//	epsi := k2 / (2*(1+math.Sqrt(1-k2)) - k2)
//	A1 := a * (1 + ellA1m1f(epsi)) * (1 - epsi) / (1 + epsi)
//	C1a := ellC1f(epsi)
...
//	sγ2 := sγ0
//	cγ2 := cγ0 * cσ2
//	lon12 := math.Atan2(sλ2*cλ1-cλ2*sλ1, cλ2*cλ1+sλ2*sλ1)
//	lon12 *= 180 / math.Pi
//	lon2 := lon1 + lon12
//	if lon2 > 180 {
//		lon2 -= 360
...
//		lon2 += 360
//	}
//	lat2 := math.Atan2(sβ2, f1*cβ2)
//	lat2 *= 180 / math.Pi
//	p2 = Geo(lat2, lon2)
//	α2 = math.Atan2(sγ2, cγ2*math.Sqrt(1-e2*cβ2*cβ2))
//	α2 *= 180 / math.Pi
//	return
//}
